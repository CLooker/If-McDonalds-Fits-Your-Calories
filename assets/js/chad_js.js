  ...
  function showAllMealsAndPictures(howManyMeals) {
    ...
    function showAllMeals(howManyMeals) {
      for (let mealNumber = 1; mealNumber <= howManyMeals; mealNumber++) {
        ...
        $(`#meal${mealNumber}Calories`)
          .html("Calories: ")
          .append(localStorage.getItem(`meal${mealNumber}Calories`))
          .append("<br>")
          .append("Protein: ")
          .append(localStorage.getItem(`meal${mealNumber}Protein`))
          .append("<br>")
          .append("Carbs: ")
          .append(localStorage.getItem(`meal${mealNumber}Carbs`))
          .append("<br>")
          .append("Fat: ")
    ...
    function getMealPictures(howManyMeals) {
      for (let i = 1; i <= howManyMeals; i++) {
        $(`#meal${i}Picture`).attr(
          "src",
          localStorage.getItem(`meal${i}ImgSrc`)
        );
      }
    }
  }
    ...
    function getFood() {
      ...
      $.ajax({
        ...
        .done(function(response) {
          ...
          let mealItemCalories =
              response.hits[randomAjaxMealIndex].fields.nf_calories,
            mealItemProtein =
              response.hits[randomAjaxMealIndex].fields.nf_protein,
            mealItemCarbs =
              response.hits[randomAjaxMealIndex].fields.nf_total_carbohydrate,
            mealItemFat =
              response.hits[randomAjaxMealIndex].fields.nf_total_fat;
          ...
          function decideNextItem() {
            ...
            function locallyStoreAllMeals(howManyMeals) {
              for (
                ...
                localStorage.setItem(
                  `meal${mealNumber}Calories`,
                  meals[`meal${mealNumber}`].calories
                );
                localStorage.setItem(
                  `meal${mealNumber}Protein`,
                  meals[`meal${mealNumber}`].protein
                );
                localStorage.setItem(
                  `meal${mealNumber}Carbs`,
                  meals[`meal${mealNumber}`].carbs
                );
                localStorage.setItem(
                  `meal${mealNumber}Fat`,
            ...
            function setAllMealImages(howManyMeals) {
              function setOneMealImage(mealNumber) {
                ...
                let getImage = meals[`meal${mealNumber}`].meal[0].item_name
                  .toLowerCase()
                  .replace(/\s+/g, "");

                for (let key in mealImageSrc) {
                  if (getImage.indexOf(key) !== -1) {
                    localStorage.setItem(
                      `meal${mealNumber}ImgSrc`,
                      mealImageSrc[key]
                    );
                    break;
                  } else {
